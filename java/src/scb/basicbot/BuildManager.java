	...
	private Position getSeedPositionFromSeedLocationStrategy(BuildOrderItem.SeedPositionStrategy seedLocationStrategy) {
		...
		case MainBaseBackYard:
			...
			if (tempBaseLocation != null && tempChokePoint != null) {
				...
				bx = tempBaseLocation.getTilePosition().getX() - (int)(d * Math.cos(theta) / BotConfig.TILE_SIZE);
				by = tempBaseLocation.getTilePosition().getY() + (int)(d * Math.sin(theta) / BotConfig.TILE_SIZE);
				//std::cout << "i";
				...
				if (!tempTilePosition.isValid() || !MyBotModule.Broodwar.isBuildable(tempTilePosition.getX(), tempTilePosition.getY(), false) || tempBaseRegion != BWTA.getRegion(new Position(bx*BotConfig.TILE_SIZE, by*BotConfig.TILE_SIZE))) {
					...
					bx = tempBaseLocation.getTilePosition().getX() + (int)(d * Math.sin(theta) / BotConfig.TILE_SIZE);
					by = tempBaseLocation.getTilePosition().getY() + (int)(d * Math.cos(theta) / BotConfig.TILE_SIZE);
					tempTilePosition = new TilePosition(bx, by);
					// std::cout << "ConstructionPlaceFinder MainBaseBackYard tempTilePosition " << tempTilePosition.x << "," << tempTilePosition.y << std::endl;
					//std::cout << "m";
				...
				if (currentItem.metaType.isUnit()) {
					...
					if (!isDeadlockCase && unitType.isBuilding() && unitType.requiresPsi()
						...
						List<Unit> ourUnits = MyBotModule.Broodwar
								.getUnitsInRadius(currentItem.seedLocation.toPosition(), 4 * BotConfig.TILE_SIZE);
