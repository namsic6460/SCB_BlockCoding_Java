...
public class ConstructionPlaceFinder {
	...
	public final TilePosition getBuildLocationWithSeedPositionAndStrategy(UnitType buildingType, TilePosition seedPosition, BuildOrderItem.SeedPositionStrategy seedPositionStrategy)
	{
		...
		// seedPosition 을 입력하지 않은 경우
			...
			case MainBaseBackYard:
				...
				bx = tempBaseLocation.getTilePosition().getX() - (int)(d * Math.cos(t) / BotConfig.TILE_SIZE);
				by = tempBaseLocation.getTilePosition().getY() + (int)(d * Math.sin(t) / BotConfig.TILE_SIZE);
				//std::cout << "i";
				...
				if (!tempTilePosition.isValid() || !MyBotModule.Broodwar.isBuildable(tempTilePosition.getX(), tempTilePosition.getY(), false) || tempBaseRegion != BWTA.getRegion(new Position(bx*BotConfig.TILE_SIZE, by*BotConfig.TILE_SIZE))) {
					//std::cout << "l";

					// BaseLocation 에서 ChokePoint 방향에 대해 오른쪽으로 90도 꺾은 방향의 Back Yard : 데카르트 좌표계에서 (cos(t-90) = sin(t),   sin(t-90) = - cos(t))
					bx = tempBaseLocation.getTilePosition().getX() + (int)(d * Math.sin(t) / BotConfig.TILE_SIZE);
					by = tempBaseLocation.getTilePosition().getY() + (int)(d * Math.cos(t) / BotConfig.TILE_SIZE);
					tempTilePosition = new TilePosition(bx, by);
					// std::cout << "ConstructionPlaceFinder MainBaseBackYard tempTilePosition " << tempTilePosition.x << "," << tempTilePosition.y << std::endl;
					//std::cout << "m";
	...
	public final TilePosition getBuildLocationNear(UnitType buildingType, TilePosition desiredPosition)
	{
		...
		int buildingGapSpace = BotConfig.BuildingSpacing;
		...
		if (buildingType.isResourceDepot()) {
			buildingGapSpace = BotConfig.BuildingResourceDepotSpacing;		
		}
			...
			if (numPylons < 3) {
				buildingGapSpace = BotConfig.BuildingPylonEarlyStageSpacing;
			}
			else {
				buildingGapSpace = BotConfig.BuildingPylonSpacing;
			}
		}
		else if (buildingType == UnitType.Terran_Supply_Depot) {
			buildingGapSpace = BotConfig.BuildingSupplyDepotSpacing;
		}
			...
			buildingGapSpace = BotConfig.BuildingDefenseTowerSpacing;
		}
