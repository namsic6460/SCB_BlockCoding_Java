...
public class MyBotModule extends DefaultBWListener {
	...
	public void onStart() {
		...
		if (BotConfig.EnableCompleteMapInformation) {
			Broodwar.enableFlag(Enum.CompleteMapInformation.getValue());
		}

		if (BotConfig.EnableUserInput) {
		...
		Broodwar.setLocalSpeed(BotConfig.SetLocalSpeed);
		// frameskip을 늘리면 화면 표시도 업데이트 안하므로 훨씬 빠르다
		Broodwar.setFrameSkip(BotConfig.SetFrameSkip);
